...
(function(mod) {
    ...
    var luaparse = (function() {
        ...
        function readLongString(isComment) {
            ...
            while (index < length) {
                ...
                // its counter.
                if (isLineTerminator(input.charCodeAt(index))) {consumeEOL();index--}
    ...
    CodeMirror.registerHelper("lint", "lua", function (text, options) {
        try {
            luaparse.parse(text);
            return []
        } catch (error) {
            if (error instanceof SyntaxError) {
                return [{
                    ...
                    to: CodeMirror.Pos(error.line - 1, error.column + (error.value ? error.value.length : 1)),
                }]
            }
        }
    });
